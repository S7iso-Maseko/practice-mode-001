...
export const getAllCompletedTasks = async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.send({ error: "Error while fetching completed tasks" });
        throw error;
    }
}
...
export const getTasksForToday = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const tasks = await Task.find({ user: userId, date: todaysISODate.toISOString() });
        res.send(tasks);
    } catch (error) {
...
export const editTask = async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.send({ error: "Error while editing task" });
        throw error;
    }
}